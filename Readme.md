# 🚀 Xtreme Load Tester Pro v4.6.0 — Pure Power Edition

**High-concurrency HTTP stress testing engine in pure C# — real-time latency analytics, stealth mode, p95/p99 metrics.**
...
[![Version](https://img.shields.io/badge/Version-v4.6.0-blueviolet)](#)

---

## 🎯 What is this?

Xtreme Load Tester Pro is a **command-line HTTP stress testing tool** built for raw performance. It spins up thousands of concurrent workers, rotates User-Agents, tracks latency per request and delivers a full p50/p95/max report when done.
...
```
User Input
  └─ URL, workers, timeout, stealth delay, duration
        │
        ▼
┌─────────────────────────────────────────────┐
│         SocketsHttpHandler                  │
...
- **Stealth delay** — configurable random delay between requests per worker (`min max ms`) to simulate real traffic patterns and bypass naive rate limiters
...
- **Graceful shutdown** — `CancellationTokenSource` ensures all workers stop cleanly when time runs out or user interrupts
...
- **Language:** C# (.NET 8+)
- **HTTP:** `SocketsHttpHandler` + `HttpClient`
...
🧵 Workers:             500
⏳ Timeout (s):         5
🕵️ Stealth Delay:       10 50       ← random delay 10–50ms per worker
⏱ Duration (s):         30
```
...
Set `Stealth Delay` to a `min max` range (in milliseconds) to randomize the delay between requests per worker. This simulates organic traffic patterns and helps test how your infrastructure handles bursty vs. sustained load.
...
Each worker picks an independent random delay — with 500 workers and `10 50ms` delay, you still generate thousands of requests per second while bypassing simple "requests per second" rate limiters.
...
- **Infrastructure bottleneck detection** — identify where your stack saturates (load balancer, app server, DB)
- **p95 latency profiling** — measure real-world latency distribution under load, not just averages
- **WAF bypass testing** — test how your Shield-X or other WAF handles high-concurrency traffic with rotating User-Agents
...
Xtreme Load Tester Pro to narzędzie do testów obciążeniowych HTTP napisane w C#. Uruchamia N równoległych workerów opartych na `SocketsHttpHandler` z limitem 10 000 połączeń per serwer i wsparciem HTTP/2. Każdy worker rotuje losowo między trzema User-Agentami (Chrome, Firefox, Safari Mobile), dodaje nagłówki `Accept` i `Referer` oraz opcjonalnie odczekuje losowy czas przed każdym requestem (tryb stealth). Latencja każdego żądania jest mierzona przez `Stopwatch` i zbierana do kolejki (max 100 000 sampli) — po zakończeniu testu generowany jest raport z metrykami p50, p95 i max. Błędy są kategoryzowane per typ (HTTP 4xx/5xx, Timeout, błąd sieci) i wyświetlane w raporcie końcowym.
